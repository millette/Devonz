...
export function useDataOperations({
  ...
  const showProgress = useCallback((message: string, percent: number) => {
    ...
    toast.loading(`${message} (${percent}%)`, {
      duration: 3000,
      id: 'progress-toast', // Use the same ID for all progress messages
    });
  }, []);
  ...
  const handleExportAllChats = useCallback(async () => {
    ...
    logger.debug('Export: Using database', {
      name: db.name,
      version: db.version,
    ...
    try {
      ...
      const directChats = await new Promise<unknown[]>((resolve, reject) => {
        try {
          ...
          request.onsuccess = () => {
            logger.debug(`Found ${request.result ? request.result.length : 0} chats directly from database`);
      ...
      logger.debug(`Preparing to export ${exportData.chats.length} chats`);
  ...
  const handleExportSelectedChats = useCallback(
    async (chatIds: string[]) => {
      ...
      try {
        ...
        logger.debug(`Retrieved ${filteredChats.length} chats for export`);
  ...
  const handleImportAPIKeys = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        document.cookie = `apiKeys=${encodeURIComponent(apiKeysJson)}; path=/; max-age=31536000; SameSite=Strict`;
